...
void Camera::HandleInputs(GLFWwindow* pWindowID){

	// Handles key inputs
	if (glfwGetKey(pWindowID, GLFW_KEY_W) == GLFW_PRESS)
	{
		cPosition += cSpeed * cOrientation;
	}
	if (glfwGetKey(pWindowID, GLFW_KEY_A) == GLFW_PRESS)
	{
		cPosition += cSpeed * -glm::normalize(glm::cross(cOrientation, cUp));
	}
	...
	{
		// Hides mouse cursor
		glfwSetInputMode(pWindowID, GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
